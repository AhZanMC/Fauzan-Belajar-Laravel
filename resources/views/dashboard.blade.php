            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col">
                ...
                <div class="h-48 bg-gray-200 flex items-center justify-center relative group">
                    @if($item->image)
                        ...
                        <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->item_name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300">
                    @else
                        <!-- Tampilkan Placeholder (SVG Lama) -->
                        <svg class="w-12 h-12 text-gray-400 group-hover:scale-110 transition-transform duration-300" ...>...</svg>
                    @endif
